...
const API_CONFIG = {
    ...
    CREATE_TASK: 'http://localhost:8080/assignment/post',
    UPDATE_TASK: 'http://localhost:8080/assignment/update',
    UPLOAD: 'http://localhost:8080/assignment/upload'
};
...
function initUI() {
    ...
    if (btnSubmitTask) {
        btnSubmitTask.addEventListener('click', async () => {
            ...
            try {
                // 1. Create the task
                const res = await authFetch(API_CONFIG.CREATE_TASK, {
                    method: 'POST',
                    ...
                    body: JSON.stringify({
                        ...
                        status: 'pending',
                        priority: 1,
                        category_ids: [] // Simplified for now
                    })
                });
                ...
                if (fileInput.files.length > 0) {
                    ...
                    await authFetch(API_CONFIG.UPLOAD, {
                        method: 'POST',
                        body: formData
                    });
                }
...
async function loadApp() {
    ...
    try {
        const [settingsRes, dataRes, userRes] = await Promise.allSettled([
        ...
        if (settingsRes.status === 'fulfilled' && settingsRes.value.ok) {
            appState.settings = await settingsRes.value.json();
            console.log('Settings loaded');
        } else {
            console.error('Failed to load settings', settingsRes.reason || 'Not OK');
            // Fallback settings
            appState.settings = { ui: { title: 'vFAST', dashboard_header: 'Academic Dashboard' } };
        }
        
        if (dataRes.status === 'fulfilled' && dataRes.value.ok) {
            const rawTasks = await dataRes.value.json();
            console.log('Tasks fetched:', rawTasks.length);
            // Map backend tasks to frontend format
            appState.tasks = rawTasks.map(t => ({
                id: t.task_id,
                title: t.title,
                description: t.description,
                deadline: t.deadline ? t.deadline.split('T')[0] : new Date().toISOString().split('T')[0],
                status_id: mapStatusToId(t.status),
                tags: t.categories ? t.categories.map(c => c.category_name) : []
            }));
        } else {
            console.error('Failed to load tasks', dataRes.reason || 'Not OK');
        }

        if (userRes.status === 'fulfilled' && userRes.value.ok) {
            appState.user = await userRes.value.json();
            console.log('User profile loaded');
        } else {
            console.error('Failed to load user profile', userRes.reason || 'Not OK');
            appState.user = { name: 'User', initials: 'U' };
        }

        renderUI();
        updateStats();
        renderTasks();
        console.log('App loaded and rendered');
    } catch (error) {
...
async function handleStatusChange(taskId, newStatusId) {
    try {
        const res = await authFetch(`${API_CONFIG.UPDATE_TASK}/${taskId}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                status: mapIdToStatus(newStatusId)
            })
        });
